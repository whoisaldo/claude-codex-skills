---
name: codex-image
description: Generate real raster images (PNG) via the Codex CLI's built-in image_gen tool — hero images, marketing visuals, illustrations, mascots, textures, backgrounds, product/device mockups, concept art, og:image cards, empty-state art, transparent cutouts. Use when a task needs an actual picture that would otherwise be faked with hand-written SVG, CSS gradients, emoji, or a placeholder box. Do NOT use for building or changing UI code, layout, CSS, components, data charts, existing SVG icon systems, or screenshots of a real running app. Do NOT use when the deliverable is a 3D object or scene, or a vector object, that must be rotated, animated part by part, or embedded as geometry (three.js/WebGL, animatable SVG): that is the codex-3d skill.
---
...
Auth is the user's existing `codex login` (a ChatGPT account). **No
`OPENAI_API_KEY` is needed**; usage goes through their ChatGPT plan's Codex
limits, not a metered API bill. Prefer generating a real asset over faking one.
...
- **Screenshots of the real app.** Use a real browser tool.
...
- **Charts and data visualization.** Real numbers require a real chart library.
  A generated chart has invented data.
...
- **A 3D object or an animatable vector object.** If it must be rotated,
  lit in real time, animated part by part (wheels spin, lid opens), or
  dropped into a three.js/WebGL scene, use the `codex-3d` skill. A flat
  picture of a 3D-looking thing is still this skill.
...
> → use this skill. If it should be *turned, moved, or animated as an object*
> → `codex-3d`.

...
**Auth:** uses your existing `codex login`. No `OPENAI_API_KEY` and no
separate API bill; usage goes through your ChatGPT plan.
...
ln -sfn "$(pwd)/claude-codex-skills/codex-image" ~/.claude/skills/codex-image
```

Requires `codex` on `PATH` and logged in (`codex login`), and Python 3.
Check that image generation is enabled with
`codex features list | grep image_generation`, which should read `stable  true`.
